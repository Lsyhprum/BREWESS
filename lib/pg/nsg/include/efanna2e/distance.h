...
namespace efanna2e{
  ...
  class DistancePQ : public Distance{
  public:
    float compare(const float* a, const float* b, unsigned size) const {
      return 0;
    }
    
    float compare2(const uint8_t* a, const float* lut, unsigned num_chunks) const {
      float dis = 0;
      unsigned dict_size = 256;
      for (size_t m = 0; m < num_chunks; m+=4) {
          float dism = 0;
          dism  = lut[*a++]; lut += dict_size;
          dism += lut[*a++]; lut += dict_size;
          dism += lut[*a++]; lut += dict_size;
          dism += lut[*a++]; lut += dict_size;
          dis += dism;
