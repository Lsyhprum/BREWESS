...
#pragma omp for schedule(dynamic, 100)
  ...
  void IndexNSG::BatchSearchWithCodes(const float *luts, const size_t batchsize, 
      const size_t num_chunks, const size_t dict_size, const uint8_t *x, size_t k, 
      const Parameters &parameters, 
      const float *query, size_t dim,
      int *nn, 
      unsigned int*indices,
      unsigned int*heap_his,
      size_t candsize,
      float *heap_d) {
    ...
    std::vector<unsigned> tmp2(candsize, 0);
    std::vector<float> tmp3(candsize, 0);

    for (size_t i = 0; i < batchsize; i ++) {
      SearchWithCodes(luts + (i * num_chunks * dict_size), k, parameters, &query[i * dim], dim, nn + i * k, tmp.data(), tmp2.data(), tmp3.data());
      for(size_t j = 0; j < k; j ++) {
        indices[i * k + j] = tmp[j];
      }
      for(unsigned j = 0; j < candsize; j ++) {
        heap_his[i * candsize + j] = tmp2[j];
      }
      for(unsigned j = 0; j < candsize; j ++) {
        heap_d[i * candsize + j] = tmp3[j];
      }
    }
    std::vector<unsigned>().swap(tmp);
    std::vector<unsigned>().swap(tmp2);
    std::vector<float>().swap(tmp3);
  }

  void IndexNSG::SearchWithCodes(const float *lut, size_t K,
                        const Parameters &parameters, unsigned *indices) {
    const unsigned L = parameters.Get<unsigned>("L_search");
    // code_ = x;
    DistancePQ *dis_ = new DistancePQ();
    ...
    for (unsigned i = 0; i < init_ids.size(); i++)
    {
      unsigned id = init_ids[i];
      float dist =
           dis_->compare2(code_ + dimension_ * id, lut, (unsigned)dimension_);
    ...
    for (size_t i = 0; i < K; i++)
    {
      ...
      if(retset[i].id > 1000000) {
        std::cout << "result val error" << std::endl;
      }
    }      
    std::vector<Neighbor>().swap(retset);
    std::vector<unsigned>().swap(init_ids);          
  }

  void IndexNSG::SearchWithCodes(const float *lut,
            size_t K,
            const Parameters &parameters,
            const float *query, size_t dim,
            int *nn,
    ...
    for (unsigned i = 0; i < init_ids.size(); i++)
    {
      ...
      float dist =
           dis_->compare2(code_ + dimension_ * id, lut, (unsigned)dimension_);
      float dist2 =
           distance_->compare(data_ + dim * id, query, (unsigned)dim);
      retset[i] = Neighbor(id, dist, true, dist2);
      // flags[id] = true;
    }
    ...
    while (k < (int)L)
    {
      ...
      if (retset[k].flag)
      {
        if (k < K) {
          for(pos = 0; pos < K; pos ++) {
            heap_his[1 + (times % 50) * (K + neighbor_r + 1) + 1 + pos] = retset[pos].id;
            heap_d[1 + (times % 50) * (K + neighbor_r + 1) + 1 + pos] = retset[pos].distance2;
          }
        }
        ...
        for (unsigned m = 0; m < final_graph_[n].size(); ++m)
        {
          ...
          if (dist >= retset[L - 1].distance)
            continue;
          Neighbor nn(id, dist, true, dist2);
          int r = InsertIntoPool(retset.data(), L, nn);
          ...
          if(k < K)
            if (m < neighbor_r) {
              heap_his[1 + (times % 50) * (K+neighbor_r+1) + pos + 1] = id;
              heap_d[1 + (times % 50) * (K+neighbor_r+1) + pos + 1] = dist2;
              pos += 1;
            }
        }

        if(k < K) {
          heap_his[1 + (times % 50) * (K+neighbor_r+1)] = pos; // 数量
          heap_d[1 + (times % 50) * (K+neighbor_r+1)] = pos;  // 数量
          times += 1;
        }
      }
      if (nk <= k)
    ...
    heap_his[0] = times > 50 ? 50 : times;   
    heap_d[0] = times > 50 ? 50 : times;
  }
  ...
  void IndexNSG::SearchWithOptGraph(const float *origin_data, const float *origin_query, const float *query, unsigned origin_dim, size_t K, const Parameters &parameters, int *indices) {
    unsigned L = parameters.Get<unsigned>("L_search");
    DistanceFastL2 *dist_fast = (DistanceFastL2 *)distance_;
    ...
    std::vector<Neighbor> retset2;
    for(size_t i = 0; i < retset.size(); i ++) {
      const float *point = origin_data + origin_dim * retset[i].id;
      float cur_norm = dist_fast->norm(point, origin_dim);
      float dist = dist_fast->compare(origin_query, point, cur_norm, origin_dim);
      //float dist = distance_->compare(origin_data + origin_dim * retset[i].id, origin_query, origin_dim);
      //std::cout << retset[i].id << "|" << retset[i].distance << "|" << dist << " ";
      retset[i].distance = dist;
      retset2.push_back(retset[i]);
    }
    //std::cout << std::endl;

    std::sort(retset2.begin(), retset2.end());

    for (size_t i = 0; i < K; i++)
    {
      indices[i] = retset2[i].id;
    }
  }
